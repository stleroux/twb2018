   ...
   @if($users->count() > 0)
      <div class="panel panel-primary">
         ...
         <div class="well well-sm text text-center">
            <a href="{{ route('backend.users.inactiveUsers') }}" class="{{ Request::is('backend/users/inactiveUsers') ? "btn-primary": "btn-default" }} btn btn-sm">All</a>
            @foreach($letters as $value)
               <a href="{{ route('backend.users.inactiveUsers', $value) }}" class="{{ Request::is('backend/users/inactiveUsers/'.$value) ? "btn-primary": "btn-default" }} btn btn-sm">{{ strtoupper($value) }}</a>
            @endforeach
         </div>
