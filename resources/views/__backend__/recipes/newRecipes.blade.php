   ...
   @if($recipes->count() > 0)
      <div class="panel panel-primary">
         ...
         <div class="well well-sm text text-center">
            <a href="{{ route('backend.recipes.newRecipes') }}" class="{{ Request::is('backend/recipes/newRecipes') ? "btn-primary": "btn-default" }} btn btn-sm">All</a>
            @foreach($letters as $value)
               <a href="{{ route('backend.recipes.newRecipes', $value) }}" class="{{ Request::is('backend/recipes/newRecipes/'.$value) ? "btn-primary": "btn-default" }} btn btn-sm">{{ strtoupper($value) }}</a>
            @endforeach
         </div>
