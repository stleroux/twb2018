...
@section('breadcrumb')
   ...
   <li class="active"><span>Future Recipes</span></li>
@stop
...
{{-- <form method="POST" action="{{ route('backend.articles.trashAll') }}"> --}}
   ...
   @if($recipes->count() > 0)
         ...
         <div class="panel-heading" style="position:relative;">
            <h3 class="panel-title">
               ...
               <a href="#" id="popover" data-toggle="popover" data-trigger="hover"
                  data-title="Future Recipes"
                  data-content="These recipes are set to be available at a future date.">
                  <i class="fa fa-question-circle" aria-hidden="true"></i> Future Recipes
               </a>
      ...
      <div class="well well-sm text text-center">
         <a href="{{ route('backend.recipes.future') }}" class="{{ Request::is('backend/recipes/future') ? "btn-primary": "btn-default" }} btn btn-sm">All</a>
         @foreach($letters as $value)
            <a href="{{ route('backend.recipes.future', $value) }}" class="{{ Request::is('backend/recipes/future/'.$value) ? "btn-primary": "btn-default" }} btn btn-sm">{{ strtoupper($value) }}</a>
         @endforeach
      </div>
    ...
    @else
      <div class="panel panel-primary">
         <div class="panel-heading">
            <h3 class="panel-title">Future Articles</h3>
         </div>
