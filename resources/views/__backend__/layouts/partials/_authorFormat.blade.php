...
@if (Auth::check())
   ...
   {{-- @if (Session::get('authorFormat') == 1) --}}
      ...
      <a href="" data-toggle="modal" data-target="#viewAuthorModal{{ $model->id }}">{{ $model->user->username }}</a>
      {{-- <a href="" data-toggle="modal" data-target="#viewAuthorModal">{{ $article->user->username }}</a> --}}
   ...
   {{-- @elseif (Session::get('authorFormat') == 2) --}}
      ...
      <a href="" data-toggle="modal" data-target="#viewAuthorModal{{ $model->id }}">{{ $model->user->profile->last_name }}, {{ $model->user->profile->first_name }}</a>
      {{-- @include('frontend.modals.authorModal', ['model'=>$model]) --}}
   ...
   {{-- @elseif (Session::get('authorFormat') == 3) --}}
      ...
      <a href="" data-toggle="modal" data-target="#viewAuthorModal{{ $model->id }}">{{ $model->user->profile->first_name }} {{ $model->user->profile->last_name }}</a>
      {{-- @include('frontend.modals.authorModal', ['model'=>$model]) --}}
   @endif

   @include('backend.modals.authorModal', ['model'=>$model])
