{{-- <div class="panel panel-{{ teamScores($game, 2)->count() < teamScores($game, 1)->count() ? 'warning' : 'primary' }}"> --}}
	@if($game->type - zeroOneTeamScores($game, 2)->sum('score') == 0 )
		<div class="panel panel-success">
	@elseif($game->type - zeroOneTeamScores($game, 1)->sum('score') == 0 )
		<div class="panel panel-primary">
	@else
		<div class="panel panel-{{ zeroOneTeamScores($game, 2)->count() < zeroOneTeamScores($game, 1)->count() ? 'warning' : 'primary' }}">
	@endif
			...
			{!! Form::open(['route' => 'darts.01.scores.teams.store']) !!}
				...
				<div class="col-xs-12 col-sm-6">
					<div class="btn-group btn-group-vertical {{ $errors->has('user_id') ? 'has-error' : '' }}" data-toggle="buttons">
						@foreach(zeroOneTeamPlayers($game, 2) as $user)
							{{-- {{ $user->shooting_order }}
							{!! nextShot($game) !!} --}}
					...
					@else
						<div class="form-group {{ $errors->has('score2') ? 'has-error' : '' }}" >
							@if(zeroOneNextShot($game) % 2 == 0)
								<input class="form-control input-lg" type="text" name="score2" style="text-align: center" autofocus />
							@else
								<input class="form-control input-lg" type="text" name="score2" style="text-align: center" />
							@endif
						</div>
						<br />
