   ...
   @if($posts->count() > 0)
      <div class="panel panel-primary">
         ...
         <div class="well well-sm text text-center">
            <a href="{{ route('posts.index') }}" class="{{ Request::is('backend/posts') ? "btn-primary": "btn-default" }} btn btn-sm">All</a>
            @foreach($letters as $value)
               <a href="{{ route('posts.index', $value) }}" class="{{ Request::is('backend/posts/'.$value) ? "btn-primary": "btn-default" }} btn btn-sm">{{ strtoupper($value) }}</a>
            @endforeach
         </div>
