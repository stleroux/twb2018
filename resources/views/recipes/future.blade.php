...
{{-- <form method="POST" action="{{ route('articles.trashAll') }}"> --}}
   ...
   @if($recipes->count() > 0)
      ...
      <div class="well well-sm text text-center">
         <a href="{{ route('recipes.future') }}" class="{{ Request::is('backend/recipes/future') ? "btn-primary": "btn-default" }} btn btn-sm">All</a>
         @foreach($letters as $value)
            <a href="{{ route('recipes.future', $value) }}" class="{{ Request::is('backend/recipes/future/'.$value) ? "btn-primary": "btn-default" }} btn btn-sm">{{ strtoupper($value) }}</a>
         @endforeach
      </div>
